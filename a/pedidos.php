...
ini_set('display_errors', 0); // Desativado para evitar HTML em respostas JSON
...
class DatabaseConnection {
    ...
    private function connect() {
        try {
            ...
            // Se o MySQL está na mesma máquina do Apache/PHP, ele pode aceitar apenas conexões em 127.0.0.1.
            // Conectar via IP da máquina (ex: 192.168.x.x) pode falhar dependendo do bind-address.
            ...
            if ($this->conn->connect_error) {
                throw new Exception(
                    "Erro na conexão: " . $this->conn->connect_error
                    . " | host_configurado=" . $configuredHost
                    . " | host_efetivo=" . $effectiveHost
                    . " | porta=" . $this->port
                    . " | server_addr=" . ($serverAddr ?: 'N/A')
                );
            }
        } catch (Exception $e) {
...
class PedidosManager {
    ...
    public function __construct() {
        ...
        if (file_exists('WhatsAppSessionManager.php')) {
            ...
            error_log("WhatsAppSessionManager.php não encontrado");
        }
    }
    ...
    private function inicializarColunasExtra() {
        $result = $this->conn->query(
            "SELECT COUNT(*) AS cnt FROM INFORMATION_SCHEMA.COLUMNS
             WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'usuario' AND COLUMN_NAME = 'telefone'"
        );
        if ($result && $result->fetch_assoc()['cnt'] == 0) {
            $this->conn->query("ALTER TABLE usuario ADD COLUMN telefone VARCHAR(20) NULL DEFAULT NULL");
        }
    }

    private function buscarTelefoneSolicitante($nomeUsuario) {
        try {
            $stmt = $this->conn->prepare("SELECT telefone FROM usuario WHERE Usuario = ? LIMIT 1");
            $stmt->bind_param("s", $nomeUsuario);
            $stmt->execute();
            $row = $stmt->get_result()->fetch_assoc();
            return !empty($row['telefone']) ? $row['telefone'] : null;
        } catch (Exception $e) {
            error_log("Erro ao buscar telefone do solicitante: " . $e->getMessage());
            return null;
        }
    }
    ...
    public function getFornecedores() {
        try {
            $fornecedores = [];
            $stmt = $this->conn->prepare("SELECT id, Nome, Contato FROM fornecedor ORDER BY Nome ASC");
            $stmt->execute();
    ...
    public function adicionarPedido($dados) {
        try {
            ...
            $stmt->bind_param("ssisssss", $produto, $fornecedor, $quantidade, $unidade, $obs, $dataHora, $usuario, $marceneiro);
    ...
    private function gerarMensagemWhatsAppRH($produto, $quantidade, $unidade, $usuario, $marceneiro, $obs) {
        $tipoMaterial = $this->detectarTipoMaterial($produto, $unidade, $obs);

        if ($tipoMaterial === 'LÂMINA') {
            $header = "📜✨ *NOVO PEDIDO DE LÂMINA* ✨📜";
            $emojiTipo = "🪵";
        } else {
            $header = "📦🚛 *NOVO PEDIDO DE MDF / DIVERSOS* 🚛📦";
            $emojiTipo = "📦";
        }

        return "$header\n\n"
            . "$emojiTipo *Produto:* $produto\n"
            . "🔹 *Tipo:* $tipoMaterial\n"
    ...
    private function gerarMensagemConfirmacaoRH($pedido) {
        $tipoMaterial = $this->detectarTipoMaterial($pedido['NomeProduto'], $pedido['Unidade'], $pedido['Observacao'] ?? '');
        $dataHoraPedido = !empty($pedido['DataPedido'])
            ? date('d/m/Y H:i', strtotime($pedido['DataPedido']))
            : '—';

        return "✅ *PEDIDO CONFIRMADO — ENVIADO AO FORNECEDOR*\n\n"
            ...
            . "📅 *Pedido em:* $dataHoraPedido\n"
            . "🕐 *Confirmado em:* " . date('d/m/Y H:i') . "\n\n"
            . "📊 *Acesse o sistema:* https://tinyurl.com/dkuta32r";
    }

    private function detectarTipoMaterial($produto, $unidade, $obs) {
        ...
        $texto = $normalize($produto . ' ' . $obs);
        $unidadeNormalizada = preg_replace('/\s+/', '', $normalize($unidade));

        $unidadesLamina = ['RL', 'ROLO', 'ROLOS', 'M', 'MT', 'MTS', 'METRO', 'METROS'];
        $isLamina = in_array($unidadeNormalizada, $unidadesLamina, true) || strpos($texto, 'LAMINA') !== false;
        if ($isLamina) {
            return 'LÂMINA';
        }

        if (strpos($texto, 'MDF') !== false) {
            return 'MDF';
        }
   ...
   public function gerarMensagemWhatsAppFornecedor($pedido) {
    ...
    $dataAtual = !empty($pedido['DataPedido'])
        ? date('d/m/Y \à\s H:i', strtotime($pedido['DataPedido']))
        : date('d/m/Y \à\s H:i');
    
    // Detectar tipo para personalizar cabeçalho
    $tipoMaterial = $this->detectarTipoMaterial($pedido['NomeProduto'], $pedido['Unidade'], $pedido['Observacao']);
    
    if ($tipoMaterial === 'LÂMINA') {
        $header = "📜✨ *PEDIDO DE COMPRA (LÂMINA)* ✨📜";
        $emojiProd = "🪵";
    } else {
        $header = "🏢🚛 *PEDIDO DE COMPRA (MDF/Outros)* 🚛🏢";
        $emojiProd = "📦";
    }
    
    return "$header\n\n"
        . "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        . "$emojiProd *Produto:* {$pedido['NomeProduto']}\n"
        . "🔹 *Tipo:* $tipoMaterial\n"
        . "📊 *Quantidade:* {$pedido['Quantidade']} {$pedido['Unidade']}\n"
        . "👤 *Solicitante:* {$pedido['NomeUsuario']}\n"
...
public function enviarWhatsAppFornecedor($pedidoId) {
    try {
        if (!$this->whatsappManager) {
            error_log("WhatsApp Manager não disponível");
            return ['success' => false, 'error' => 'WhatsApp Manager não disponível. Verifique a instalação.'];
        }
        ...
        if (!$pedido) {
            error_log("Pedido não encontrado: ID $pedidoId");
        ...
        if (empty($pedido['ContatoFornecedor'])) {
            error_log("Fornecedor sem telefone: " . $pedido['NomeFornecedor']);
            return ['success' => false, 'error' => 'Fornecedor não possui telefone cadastrado'];
        }
        
        $telefone = Validator::validatePhone($pedido['ContatoFornecedor']);
        if (!$telefone) {
            error_log("Telefone inválido: " . $pedido['ContatoFornecedor']);
            return ['success' => false, 'error' => 'Telefone do fornecedor inválido: ' . $pedido['ContatoFornecedor']];
        }
        ...
        // Obter sessão do usuário logado
        if (!isset($_SESSION['usuario'])) {
            error_log("Usuário não está na sessão");
            return ['success' => false, 'error' => 'Sessão expirada. Faça login novamente.'];
        }
        
        $sessaoInfo = $this->whatsappManager->obterSessaoUsuario($_SESSION['usuario']);
        if (empty($sessaoInfo['session_id'])) {
            error_log("Session ID não encontrada para usuário: " . $_SESSION['usuario']);
            return ['success' => false, 'error' => 'Sessão WhatsApp não encontrada'];
        }
        ...
        $status = $this->whatsappManager->verificarStatus($sessionId);
        if (($status['state'] ?? '') !== 'CONNECTED') {
            error_log("WhatsApp não conectado. Status: " . ($status['state'] ?? 'desconhecido'));
            return [
                'success' => false,
        ...
        error_log("Enviando WhatsApp para: $chatId via $url");
        ...
        error_log("Resposta API - HTTP $httpCode: " . substr($response, 0, 200));
        ...
        if ($httpCode === 200 && $response) {
            ...
            // Notificar SOLICITANTE que o pedido foi confirmado e enviado ao fornecedor
            $telefoneSolicitante = $this->buscarTelefoneSolicitante($pedido['NomeUsuario']);
            if (!empty($telefoneSolicitante)) {
                $mensagemConfirmacao = $this->gerarMensagemConfirmacaoRH($pedido);
                $this->enviarWhatsAppRH($mensagemConfirmacao, $telefoneSolicitante);
            } else {
                error_log("Confirmação não enviada: telefone não cadastrado para o solicitante {$pedido['NomeUsuario']}");
            }
            ...
            $errorMsg = $curlError ?: 'Erro ao enviar mensagem (HTTP ' . $httpCode . ')';
            error_log("Erro ao enviar: $errorMsg");
            return [
                'success' => false,
                'error' => $errorMsg,
        ...
        error_log("Erro ao enviar WhatsApp para fornecedor: " . $e->getMessage() . " - " . $e->getTraceAsString());
    ...
    /**
     * Atualizar fornecedor de um pedido
     */
    public function atualizarFornecedorPedido($pedidoId, $fornecedorId) {
        try {
            // Buscar nome e contato do fornecedor
            $stmt = $this->conn->prepare("SELECT Nome, Contato FROM fornecedor WHERE id = ?");
            ...
            // Atualizar apenas o nome do fornecedor no pedido
            // O contato será obtido via JOIN quando necessário
            $stmt = $this->conn->prepare("
                UPDATE pedidos 
                SET NomeFornecedor = ? 
                WHERE id = ?
            ");
            $stmt->bind_param("si", $fornecedor['Nome'], $pedidoId);
            ...
            if ($stmt->execute()) {
                ...
                return ['success' => false, 'error' => 'Erro ao atualizar: ' . $stmt->error];
            }
        } catch (Exception $e) {
            error_log("Erro ao atualizar fornecedor do pedido: " . $e->getMessage());
            return ['success' => false, 'error' => $e->getMessage()];
        }
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handler para atualizar fornecedor via AJAX
    if (isset($_POST['atualizar_fornecedor'])) {
        ...
        try {
            ...
            $fornecedorId = intval($_POST['fornecedor_id']);
            
            $resultado = $pedidosManager->atualizarFornecedorPedido($pedidoId, $fornecedorId);
            echo json_encode($resultado);
        } catch (Exception $e) {
            error_log("Erro no handler atualizar_fornecedor: " . $e->getMessage());
            echo json_encode([
                'success' => false,
    ...
    if (isset($_POST['enviar_whatsapp_fornecedor'])) {
        ...
        try {
            ...
            error_log("Erro no handler enviar_whatsapp_fornecedor: " . $e->getMessage());
            echo json_encode([
                'success' => false,
                'error' => 'Erro ao processar requisição: ' . $e->getMessage()
            ]);
    ...
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="user-info">
                ...
                <a href="fornecedores.php" class="btn btn-warning">
                    ...
                    <div class="form-group">
                        ...
                        <select name="NomeFornecedor" required class="form-control">
                            ...
                            <?php foreach ($fornecedores as $fornecedor): ?>
                                <option value="<?= htmlspecialchars($fornecedor['Nome']) ?>"><?= htmlspecialchars($fornecedor['Nome']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
                            ...
                            <tr data-pedido-id="<?= htmlspecialchars($row['id']) ?>">
                                ...
                                <td>
                                    <select class="fornecedor-select" 
                                            ...
                                            data-fornecedor-atual="<?= htmlspecialchars($row['NomeFornecedor']) ?>"
                                            onchange="atualizarFornecedor(this)">
                                        <option value="">-- Selecione Fornecedor --</option>
                                        <?php foreach($fornecedores as $fornecedor): ?>
                                            <option value="<?= htmlspecialchars($fornecedor['id']) ?>"
                                                    <?= ($fornecedor['Nome'] === $row['NomeFornecedor']) ? 'selected' : '' ?>>
