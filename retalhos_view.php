...
foreach ($lines as $index => $line) {
    ...
    $codigo_linha = trim($fields[0]);
    if (empty($codigo_linha)) {
        $codigo_linha = 'RET' . str_pad($code, 5, '0', STR_PAD_LEFT) . '-' . ($index + 1);
    }
    ...
    $retalhos[] = [
        ...
        'espessura' => $material_info['espessura'],
        'ativo' => trim($fields[1]),
...
<body>
                                    ...
                                    <td>
                                        <?php if ($retalho['disponivel'] > 0): ?>
                                            <button class="btn btn-secondary btn-small" 
                                                    onclick='openSolicitarModal(<?= json_encode($retalho) ?>)'>
                                                📲 Solicitar
                                            </button>
                                        <?php else: ?>
    ...
    <script>
        ...
        function openSolicitarModal(retalho) {
            currentRetalho = {
                codigo: retalho.codigo || '',
                codigo_material: retalho.codigo_material || '',
                material: retalho.material || '',
                maxQuantidade: retalho.disponivel || 1,
                altura: retalho.altura || 0,
                largura: retalho.largura || 0,
                descricao: retalho.descricao || '',
                espessura: retalho.espessura || ''
            };
            ...
            document.getElementById('retalhoInfo').innerHTML = `
                <div style="background: var(--nexus-bg-tertiary); padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                    ...
                    <p><strong>🏷️ Código:</strong> <span class="material-code">${currentRetalho.codigo}</span></p>
                    <p><strong>🏗️ Material:</strong> ${currentRetalho.material}</p>
                    <p><strong>📏 Dimensões:</strong> ${currentRetalho.altura} x ${currentRetalho.largura} mm</p>
                    <p><strong>📝 Descrição:</strong> ${currentRetalho.descricao}</p>
                    <p><strong>📦 Disponível:</strong> <span style="color: var(--nexus-accent-green);">${currentRetalho.maxQuantidade} peça(s)</span></p>
                </div>
            `;

            const quantidadeInput = document.getElementById('quantidade');
            quantidadeInput.max = currentRetalho.maxQuantidade;
        ...
        document.getElementById('solicitarForm').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('enviar_whatsapp_retalho.php', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (result.success) {
                    const msgSucesso = result.message || 'Solicitação enviada com sucesso via WhatsApp!';
                    alert('✅ ' + msgSucesso);
                    closeSolicitarModal();
                    location.reload();
                } else {
                    let mensagemErro = result.message || result.error || 'Erro desconhecido';
                    
                    // Se for erro de sessão WhatsApp desconectada
                    if (result.details && result.details.http_code === 404) {
                        alert('⚠️ WhatsApp Desconectado! Acesse config_whatsapp_retalhos.php para conectar.');
                    } else {
