...
foreach ($lines as $line) {
    ...
    $retalhos[] = [
        ...
        'ativo' => trim($fields[1]),
        'quantidade' => (int)trim($fields[2]), // Garantir que seja inteiro
        'altura' => trim($fields[3]),
        'largura' => trim($fields[4]),
        'descricao' => trim($fields[5]),
        'reservado' => isset($fields[6]) ? (int)trim($fields[6]) : 0, // Garantir que seja inteiro
    ];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($retalhos as &$retalho) {
        $retalho['quantidade'] = (int)$retalho['quantidade']; // Converter para inteiro
        $retalho['reservado'] = (int)$retalho['reservado'];   // Converter para inteiro
...
<body>
    <div class="container">
        <header>
            <h1>Retalhos do Material: <span class="material-info"><?= htmlspecialchars($material_name) ?></span> (Código: <?= htmlspecialchars($code) ?>)</h1>
        </header>

        <button onclick="window.location.href='materiais_view.php';">Voltar</button>
   ...
   function solicitarRetalho(codigo, material, quantidade, altura, largura, descricao) {
    ...
    // Verificação para garantir que há disponibilidade de retalhos
    if (quantidade <= 0) {
