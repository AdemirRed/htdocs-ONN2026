...
$current_utc = '2025-07-28 16:24:06'; // UTC exato fornecido
$current_user = 'AdemirRed'; // Login exato fornecido
...
// NOVO: Variáveis para processamento em lote
$lote_resultado = null;
$lote_total = 0;
$lote_sucesso = 0;
$lote_falha = 0;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Checa se veio vários códigos em lote, senão mantém o modo antigo
    if (!empty($_POST['codigos_lote'])) {
        $codigos_lote = trim($_POST['codigos_lote']);
        $codigos_array = preg_split('/\r\n|\r|\n/', $codigos_lote);
        $lote_total = 0;
        $lote_sucesso = 0;
        $lote_falha = 0;
        $lote_erros = [];
        ...
        foreach ($codigos_array as $codigo_barras) {
            $codigo_barras = trim($codigo_barras);
            if ($codigo_barras === '') continue;
            $lote_total++;
            $res = processarCodigoDeBarras($codigo_barras, $materials, $directory_retalhos, $local_time, $current_user);
            if ($res['success']) {
                $lote_sucesso++;
            } else {
                $lote_falha++;
                $lote_erros[] = [
                    ...
                    'erro' => $res['message'],
                    'detalhes' => isset($res['details']) ? $res['details'] : ''
                ];
            }
            $lote_resultados[] = $res;
        }
        ...
        $lote_resultado = [
            'total' => $lote_total,
            'sucesso' => $lote_sucesso,
            'falha' => $lote_falha,
            'erros' => $lote_erros,
            'resultados' => $lote_resultados
        ];
    }
    // Modo individual antigo (mantido)
    elseif (!empty($_POST['codigo_barras'])) {
    ...
    <div class="container">
        ...
        <div class="main-grid">
            ...
            <div class="glass-card">
                ...
                <!-- NOVO: Formulário para lote -->
                <form method="POST" id="loteForm" class="form-section" style="margin-bottom:20px;">
                    <div class="form-group">
                        <label for="codigos_lote" class="form-label">📋 Baixa em Lote (um código por linha)</label>
                        <textarea name="codigos_lote" id="codigos_lote" class="form-input" rows="5"
                        placeholder="Cole vários códigos de barras aqui, cada um em uma linha:
000009901840.002740.0
000009901840.002740.0
000009901840.002740.0" style="font-family: monospace; font-size: 14px;"></textarea>
                        <div class="decode-display" id="lote-info">
                            Total Counts: 0
                        </div>
                    </div>
                    <div class="btn-grid">
                        ...
                        <button type="button" class="btn btn-secondary" id="clearLoteBtn">
                            <span>🗑️ Limpar</span>
                        </button>
                    </div>
                </form>
                ...
                <form method="POST" id="barcodeForm" class="form-section">
                    <div class="form-group">
                        <label for="codigo_barras" class="form-label">▶ Código Individual</label>
                        <input type="text" 
                               name="codigo_barras" 
                               id="codigo_barras" 
                               class="form-input"
            ...
            <div class="glass-card">
                ...
                <?php if ($lote_resultado): ?>
                    <div class="result success">
                        <strong>📊 Resumo do Lote Processado:</strong><br><br>
                        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin: 15px 0;">
                            <div style="text-align: center; padding: 15px; background: var(--bg-tertiary); border-radius: 10px;">
                                <div style="font-size: 2em; color: var(--accent);"><?= $lote_resultado['total'] ?></div>
                                <div>Total</div>
                            </div>
                            <div style="text-align: center; padding: 15px; background: var(--bg-tertiary); border-radius: 10px;">
                                <div style="font-size: 2em; color: #4CAF50;"><?= $lote_resultado['sucesso'] ?></div>
                                <div>Sucesso</div>
                            </div>
                            <div style="text-align: center; padding: 15px; background: var(--bg-tertiary); border-radius: 10px;">
                                <div style="font-size: 2em; color: #f44336;"><?= $lote_resultado['falha'] ?></div>
                                <div>Falha</div>
                            </div>
                        </div>
                        
                        <?php if (count($lote_resultado['erros']) > 0): ?>
                            <div style="margin-top: 20px; padding: 15px; background: rgba(244, 67, 54, 0.1); border-left: 4px solid #f44336; border-radius: 5px;">
                                <strong>❌ Códigos não processados (fazer manualmente):</strong>
                                <div style="margin-top: 10px; max-height: 300px; overflow-y: auto;">
                                    <?php foreach ($lote_resultado['erros'] as $index => $erro): ?>
                                        <div style="margin: 8px 0; padding: 10px; background: var(--bg-secondary); border-radius: 5px; font-family: monospace;">
                                            <div style="font-weight: bold; color: var(--accent);"><?= $index + 1 ?>. <?= htmlspecialchars($erro['codigo']) ?></div>
                                            <div style="color: #f44336; margin-top: 5px;"><?= htmlspecialchars($erro['erro']) ?></div>
                                            <?php if ($erro['detalhes']): ?>
                                                <div style="color: var(--text-secondary); font-size: 0.9em; margin-top: 3px;"><?= htmlspecialchars($erro['detalhes']) ?></div>
                                            <?php endif; ?>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                                <div style="color: var(--text-secondary); margin-top: 15px; font-style: italic;">
                                    💡 Os códigos acima precisam ser verificados e baixados manualmente se necessário.
                                </div>
                            </div>
                        <?php else: ?>
                            <div style="margin-top: 15px; padding: 15px; background: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; border-radius: 5px;">
                                <strong>🎉 Todos os códigos foram processados com sucesso!</strong>
                            </div>
                        <?php endif; ?>
                    </div>
            ...
            <div class="management-grid">
                ...
                <div class="management-card">
                    ...
                    <div style="margin-top: 20px;">
                        <input type="number" id="materialConsulta" class="form-input no-autofocus" placeholder="Código do Material" style="margin-bottom: 15px;">
    ...
    <script>
        ...
        const loteTextarea = document.getElementById('codigos_lote');
        const loteInfo = document.getElementById('lote-info');
        const materialConsultaInput = document.getElementById('materialConsulta');
        let scanning = false;
        let stream = null;
        let barcodeDetector = null;
        let lastDetectionTime = 0;
        let debounceTimer = null;
        let autoFocusEnabled = true; // NOVA VARIÁVEL: Controla se o auto-foco está ativo
        ...
        // NOVA FUNÇÃO: Foco inteligente que respeita outros campos
        function smartFocus() {
            ...
            const hasNoAutoFocus = activeElement.classList.contains('no-autofocus');
            
            // NÃO aplica foco se:
            // - Está em modal
            // - Está em textarea
            // - Está em input com classe no-autofocus
            // - Está em botão
            // - Já está no campo principal
            if (isInModal || isTextarea || hasNoAutoFocus || isButton || activeElement === codigoInput) {
                return;
            }
            
            // Só aplica foco se não estiver em nenhum input específico
            if (!isInput) {
                codigoInput.focus();
            }
        }

        // NOVA FUNÇÃO: Desabilitar auto-foco temporariamente
        function disableAutoFocus(duration = 5000) {
        ...
        // NOVA FUNÇÃO: Atualizar informações do lote em tempo real
        function updateLoteInfo() {
            const content = loteTextarea.value.trim();
            if (!content) {
                loteInfo.innerHTML = 'Total Counts: 0';
                loteInfo.className = 'decode-display';
                return;
            }
            ...
            loteInfo.innerHTML = `Total Counts: ${count}`;
            loteInfo.className = count > 0 ? 'decode-display active' : 'decode-display';
        }
        ...
        function setupEventListeners() {
            ...
            // NOVO: Botão limpar lote
            document.getElementById('clearLoteBtn').addEventListener('click', () => {
                loteTextarea.value = '';
                updateLoteInfo();
                setTimeout(() => codigoInput.focus(), 100);
            });
            
            // NOVO: Atualização em tempo real do lote
            loteTextarea.addEventListener('input', updateLoteInfo);
            
            // NOVO: Desabilita auto-foco quando entra no textarea lote
            loteTextarea.addEventListener('focus', () => {
                disableAutoFocus(10000); // 10 segundos sem auto-foco
            });
            
            // NOVO: Desabilita auto-foco quando entra no campo de consulta
            materialConsultaInput.addEventListener('focus', () => {
                disableAutoFocus(10000); // 10 segundos sem auto-foco
            });
            ...
            document.getElementById('closeMateriaisModal').addEventListener('click', () => {
                fecharModal('materiaisModal');
                setTimeout(() => codigoInput.focus(), 100);
            });
            document.getElementById('closeRetalhosModal').addEventListener('click', () => {
                fecharModal('retalhosModal');
                setTimeout(() => codigoInput.focus(), 100);
            });
            
            // Fechar modal clicando fora
            document.getElementById('materiaisModal').addEventListener('click', (e) => {
                if (e.target.id === 'materiaisModal') {
                    fecharModal('materiaisModal');
                    setTimeout(() => codigoInput.focus(), 100);
                }
            });
            document.getElementById('retalhosModal').addEventListener('click', (e) => {
                if (e.target.id === 'retalhosModal') {
                    fecharModal('retalhosModal');
                    setTimeout(() => codigoInput.focus(), 100);
                }
            });
            ...
            materialConsultaInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') consultarRetalhos();
            });

            // NOVO: Enter no campo individual submete o form
            ...
            // NOVO: Enter no textarea do lote adiciona linha
            loteTextarea.addEventListener('keypress', (e) => {
            ...
            // NOVO: Auto-foco inteligente com intervalo maior
            setInterval(smartFocus, 3000); // A cada 3 segundos
        }
        ...
        // Funções da câmera (mantidas iguais)
        ...
        function consultarRetalhos() {
            const codigoMaterial = materialConsultaInput.value;
            ...
            fetch(`?api=retalhos&material=${codigoMaterial}`)
                ...
                .then(retalhos => {
                    ...
                    retalhos.forEach(retalho => {
                        // Monta o código de barras sintético compatível com o backend
                        const codMaterial = String(codigoMaterial).padStart(7, '0');
                        const codLargura = String(retalho.largura).padStart(7, '0');
                        const codAltura = String(retalho.altura).padStart(7, '0');
                        const codigoBarras = `${codMaterial}${codLargura}${codAltura}`;
                        ...
                        html += `
                            <div class="retalho-item">
                                ...
                                <form method="POST" style="margin-top:10px;display:inline;">
                                    ...
                                    <button type="submit" class="btn btn-primary" style="padding:8px 14px;font-size:1em;">Baixar</button>
                                </form>
                            </div>
                        `;
                    });
        ...
        // Previne submit vazio nos formulários
        ...
        document.getElementById('loteForm').addEventListener('submit', (e) => {
            const loteContent = loteTextarea.value.trim();
            if (!loteContent) {
                e.preventDefault();
                alert('Por favor, cole ao menos um código de barras para processar em lote.');
                loteTextarea.focus();
                return false;
            }
        });
        ...
        // Limpar os campos quando a página carrega para evitar reenvios
        ...
        // NOVO: Gerenciamento inteligente de foco baseado em cliques
        document.addEventListener('click', (e) => {
            // Se clicou fora de elementos interativos e não está em modal, volta pro campo principal
            const isInteractiveElement = e.target.matches('input, textarea, button, select, a, [tabindex]');
            const isInModal = e.target.closest('.modal');
            const hasNoAutoFocus = e.target.classList.contains('no-autofocus');
            
            if (!isInteractiveElement && !isInModal && !hasNoAutoFocus) {
                setTimeout(smartFocus, 100);
            }
        });

        // Intercepta teclas especiais para funcionalidades rápidas
        document.addEventListener('keydown', (e) => {
            // ESC fecha modais e volta o foco
            if (e.key === 'Escape') {
            ...
            // F2 para focar no campo de lote
            if (e.key === 'F2') {
                e.preventDefault();
                loteTextarea.focus();
                disableAutoFocus(10000);
            }
            
            // F3 para focar no campo de consulta
            if (e.key === 'F3') {
                e.preventDefault();
                materialConsultaInput.focus();
                disableAutoFocus(10000);
            }
            
            // F1 para focar no campo individual
            if (e.key === 'F1') {
                e.preventDefault();
                codigoInput.focus();
                autoFocusEnabled = true;
