...
class EditorPlanoCorte:
    ...
    def criar_interface(self):
        ...
        self.edit_panel = EditControlsPanel(
            ...
            self.xml_tree,
            self.salvar_arquivo,
            self  # Passar referência do editor para refresh
        )
    ...
    def salvar_arquivo(self):
        ...
        try:
            ...
            # DEBUG: Verificar se alterações estão no XML antes de salvar
            print("\n=== DEBUG SALVAR ===")
            root = self.xml_tree.getroot()
            for material in root.findall('.//CutItem'):
                if self._localname(material.get('type')) == 'Material':
                    for program in material.findall('.//CutItem'):
                        if self._localname(program.get('type')) == 'Program':
                            cuts = program.findall('.//Cut')
                            for i, cut in enumerate(cuts):
                                if cut.get('type') in ['P', 'Y']:
                                    print(f"Cut {i}: type={cut.get('type')}, value={cut.get('value')}")
                            break
                    break
            print("===================\n")
            ...
            xml_str = ET.tostring(self.xml_tree.getroot(), encoding='unicode', method='xml')
